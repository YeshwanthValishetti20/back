...
const Car_m = require("../models/Car_m");
...
const addMatch = async (req, res) => {
  try {
    ...
    const newMatch = new Car_m({ name, teamA, teamB, status, gender }); // Update to include teamA and teamB
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await Car_m.findById(id);
    if (!existingMatch) {
...
const getMatches = async (req, res) => {
  try {
    const matches = await Car_m.find();
    res.json(matches);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await Car_m.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await Car_m.findById(matchId);
