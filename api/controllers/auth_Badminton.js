...
const Bad_m = require("../models/Bad_m");
...
const addMatch = async (req, res) => {
  try {
    ...
    const newMatch = new Bad_m({ name, teamA, teamB, status, gender }); // Update to include teamA and teamB
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const getMatches = async (req, res) => {
  try {
    const matches = await Bad_m.find();
    res.json(matches);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await Bad_m.findById(id);
    if (!existingMatch) {
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await Bad_m.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await Bad_m.findById(matchId);
