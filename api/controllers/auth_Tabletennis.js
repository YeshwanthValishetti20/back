...
const TT_m = require("../models/TT_m");
...
const addMatch = async (req, res) => {
  try {
    ...
    const newMatch = new TT_m({ name, teamA, teamB, status, gender }); // Update to include teamA and teamB
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await TT_m.findById(id);
    if (!existingMatch) {
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await TT_m.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await TT_m.findById(matchId);
