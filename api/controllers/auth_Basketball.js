...
const Bask_m = require("../models/Bask_m");
...
const addMatch = async (req, res) => {
  try {
    ...
    const newMatch = new Bask_m({ name, teamA, teamB, status, gender }); // Update to include teamA and teamB
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const getMatches = async (req, res) => {
  try {
    const matches = await Bask_m.find();
    res.json(matches);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await Bask_m.findById(id);
    if (!existingMatch) {
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await Bask_m.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await Bask_m.findById(matchId);
