...
const addMatch = async (req, res) => {
  try {
    ...
    const newMatch = new K_m({ name, teamA, teamB, status, gender }); // Update to include teamA and teamB
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await K_m.findById(id);
    if (!existingMatch) {
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await K_m.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await K_m.findById(matchId);
